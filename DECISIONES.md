...
* **Ambigüedad:** La sección 2.4 indica que, por defecto, para cada artículo un revisor está *no interesado*. Sin embargo, la sección 2.5 establece el orden de prioridad de asignación como: `Interested -> Maybe -> sin bid -> NotInterested`. Si por defecto el estado es *no interesado*, no habría diferencia entre "sin bid" y "no interesado".
...
### 2.3. Uso de `Symbol` para los Intereses del Bidding
* **Decisión:** En `src/Bid.js`, los valores de interés (`Interested`, `Maybe`, `NotInterested`, `Conflict`) se modelaron como instancias de `Symbol` encapsuladas en un objeto congelado (`Object.freeze`).
* **Justificación:** Esto evita errores de tipeo y fallas silenciosas asociadas al uso de strings literales y proporciona una semántica de tipo enumerado estricto.

### 2.4. Invariantes en el Modelo de Dominio (`Paper` y `Review`)
...
### 2.5. Restricción de Estilo de Código (Sin Lambdas ni Funciones Anónimas)
