...
Contiene la lógica pura del algoritmo de asignación. Distribuye la carga en cuotas y procesa secuencialmente prioridades.
...
* **Algoritmo de Asignación:**
  1. Calcula la cuota por revisor de forma equitativa: `cuotaBase = Math.floor(3 * A / R)`. Distribuye el `resto` asignando una unidad más de carga a los primeros `resto` revisores.
  2. Para cada artículo, busca asignar exactamente 3 revisores.
  ...
  6. Si no es posible asignar exactamente 3 revisores a cada artículo, la transacción falla atómicamente arrojando un error y la sesión permanece en `Bidding`.
...
### 5.4. Etapa `Selection`
* Se accede a través de `closeReviewing()`, el cual verifica que se hayan recibido las 3 revisiones obligatorias para cada artículo de la sesión.
...
* **Suites de test implementadas:** 11 suites, compuestas por 50 tests totales en verde.
* **Cobertura global de código:** superior al **90%** (93.68% statements, 90.9% branches).
